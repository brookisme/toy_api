...
cp <toy_api_package>/config/*.yaml toy_api_config/
...
1. **Local Project Configs**: `./toy_api_config/config_name.yaml`
2. **Package Configs**: Built-in config from the toy_api package
...
toy_api                    # → toy_api_config/v1.yaml OR package v1.yaml

# With config name - searches both locations
toy_api my_config          # → toy_api_config/my_config.yaml OR package my_config.yaml

# List what's available
toy_api --list-configs     # Shows both local and package configs

# Set up local config directory
toy_api --init-configs     # Creates ./toy_api_config/
```

### Benefits

- **Override Defaults**: Customize package config for your project
- **No Path Hassles**: Just use config names, not full paths
- **Clear Priority**: Local config always overrides package config
